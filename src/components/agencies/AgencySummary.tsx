...
export default function AgencySummary() {
  ...
  const metrics = [
    ...
    },
    {
      ...
      prev: `${AG_FIRED_2025} из ${AG_TOTAL_2025}`,
      delta: `${EARLY_FIRED} ушли в первый год`,
      good: false,
    },
    {
  ...
  const findings = [
    ...
    {
      icon: 'UserMinus',
      title: 'Каждый шестой нанятый уходит',
      text: `Текучесть найма 2025 года — ${AG_TURNOVER_2025.toFixed(1)}%: из ${AG_TOTAL_2025} человек уволилось ${AG_FIRED_2025}. У КА ЭФИР показатель ${EFIR_TURNOVER.toFixed(1)}%, то есть основная часть потерь приходится на главного поставщика.`,
    },
    {
      icon: 'Clock',
      title: 'Потери в первый год',
      text: `${agPct(EARLY_FIRED, AG_FIRED_2025).toFixed(0)}% уволившихся не проработали года, большинство ушло на 6–12 месяце. Стоимость подбора по этим позициям пришлось платить дважды.`,
    },
    {
      icon: 'Target',
